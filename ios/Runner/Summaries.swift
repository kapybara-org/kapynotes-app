...
/// the device: no download, no account, no minutes, and the transcript never
/// leaves it. The cost is that it is conditional — the phone has to be
...
/// build on an older SDK, or a run on an older iOS, is a phone that answers
/// "unsupported" rather than one that fails to launch.
///
/// The twin of `macos/Runner/Summaries.swift`. The two runners are separate
/// targets with separate Flutter imports, which is how the rest of this app's
/// native code is arranged too.
enum Summaries {
