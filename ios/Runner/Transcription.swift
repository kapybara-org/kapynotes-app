...
/// them, and between them they cover every iPhone and iPad this app runs on:
///
///   * **`SpeechAnalyzer`**, from iOS 26. It reads the `.m4a` directly, and
...
///   * **`SFSpeechRecognizer`**, on-device, from iOS 13. Older and a
...
/// availability, and conflating them would make a phone that can transcribe
/// say it cannot.
///
...
/// on an older SDK, or a run on an older iOS, is a phone that answers with
...
enum Transcription {
  ...
  private static func transcribe(_ rawArguments: Any?, _ result: @escaping FlutterResult) {
    ...
    private static func state(for language: String?, forcing: String?) async -> String {
      if #available(iOS 26.0, *), forcing != "legacy" {
    ...
    /// the phone is set to, then this language in any region at all. A German
    ...
    private enum Failure: LocalizedError {
      ...
      var errorDescription: String? {
        ...
        case .denied:
          return "Kapy Notes was not allowed to use speech recognition. You can change that in Settings › Privacy & Security."
        }
      }
    }
  #endif
