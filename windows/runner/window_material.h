...
// Flutter paints in transparency mode have something to be thin over. Windows
// 11 calls the material acrylic and has a documented attribute for it; on
// Windows 10 the same effect comes from the composition attribute that the
// system's own Start menu and taskbar use, which is not documented but has
// held since 1803. Where neither works the window stays as it was and the
// Dart side is told, so it keeps its opaque paint rather than showing black
// through the gaps.
//
