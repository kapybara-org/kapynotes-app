...
/// no minutes, and the recording never leaves the device. Two things make it
...
///
/// It is **not** Apple Intelligence and does not need it switched on: measured
/// on a Mac with the feature off, nine English locales were already installed
/// and a five-second recording came back in 0.59 s. That is why this is a
/// separate channel from `Summaries` rather than a method on it — they have
/// different availability, and conflating them would make a Mac that can
/// transcribe say it cannot.
///
/// Everything is compiled behind `canImport` and `#available` so that a build
/// on an older SDK, or a run on an older macOS, is a Mac that answers
